...
export type WallpaperDefinition = {
  name: string;
  type: 'standalone' | 'dynamic';
  thumbnail: string;
  image?: string;
  timestamps?: {
    wallpaper?: Record<number, string>;
    theme?: Record<number, ThemeScheme>;
  };
};
...
function getAssetByName(
  ...
  const entry = Object.entries(assets).find(([assetPath]) => assetPath.includes(imageName));

  if (!entry) {
    throw new Error(`Unable to resolve ${kind} asset for "${imageName}".`);
  }

  return entry[1].default;
}

function attachWallpaperAssets<T extends Record<string, WallpaperDefinition>>(config: T) {
  for (const [wallpaperId, wallpaper] of Object.entries(config)) {
    const currentWallpaper = wallpaper as WallpaperDefinition;
    const thumbnailName = currentWallpaper.thumbnail;

    currentWallpaper.thumbnail = getAssetByName(wallpaperThumbnails, thumbnailName, 'thumbnail');
    currentWallpaper.image = getAssetByName(
      optimizedWallpapers,
      thumbnailName ?? wallpaperId,
      'image',
    );

    if (currentWallpaper.type === 'dynamic' && currentWallpaper.timestamps?.wallpaper) {
      for (const [hour, imageName] of Object.entries(currentWallpaper.timestamps.wallpaper)) {
        currentWallpaper.timestamps.wallpaper[Number(hour)] = getAssetByName(
          optimizedWallpapers,
          imageName,
          'image',
        );
...
function getScheduledValue<T>(schedule: Record<number, T> | undefined, hour: number) {
  if (!schedule) {
    return undefined;
  }

  const sortedHours = Object.keys(schedule)
    .map(Number)
    .sort((left, right) => left - right);

  const matchingHour = [...sortedHours].reverse().find((scheduledHour) => scheduledHour <= hour);

  return schedule[matchingHour ?? sortedHours.at(-1)!];
}

export function resolveWallpaperFrame(wallpaper: WallpaperDefinition, hour: number) {
  return {
    image:
      wallpaper.type === 'dynamic'
        ? getScheduledValue(wallpaper.timestamps?.wallpaper, hour) ?? wallpaper.image
        : wallpaper.image,
    theme:
      wallpaper.type === 'dynamic'
        ? getScheduledValue(wallpaper.timestamps?.theme, hour)
        : undefined,
  };
}
...
} satisfies Record<string, WallpaperDefinition>;

export type WallpaperId = keyof typeof wallpaperConfig;
export type WallpaperEntry = [WallpaperId, WallpaperDefinition];

export const wallpapers = attachWallpaperAssets({
  ...wallpaperConfig,
}) as Record<WallpaperId, WallpaperDefinition>;

const wallpaperEntries = (Object.keys(wallpapers) as WallpaperId[]).map(
  (wallpaperId) => [wallpaperId, wallpapers[wallpaperId]] as WallpaperEntry,
);

export const dynamicWallpapers = wallpaperEntries.filter(
  ([, wallpaper]) => wallpaper.type === 'dynamic',
);

export const standaloneWallpapers = wallpaperEntries.filter(
  ([, wallpaper]) => wallpaper.type === 'standalone',
);
